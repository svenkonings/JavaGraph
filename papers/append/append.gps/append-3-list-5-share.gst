<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<gxl xmlns="http://www.gupro.de/GXL/gxl-1.0.dtd">
    <graph edgemode="directed" edgeids="false" role="graph" id="append-3-list-5-share">
        <attr name="$version">
            <string>curly</string>
        </attr>
        <node id="n112"/>
        <node id="n116"/>
        <node id="n118"/>
        <node id="n110"/>
        <node id="n113"/>
        <node id="n111"/>
        <node id="n119"/>
        <node id="n122"/>
        <node id="n125"/>
        <node id="n123"/>
        <node id="n117"/>
        <node id="n115"/>
        <node id="n124"/>
        <node id="n121"/>
        <node id="n114"/>
        <node id="n120"/>
        <edge to="n112" from="n112">
            <attr name="label">
                <string>11</string>
            </attr>
        </edge>
        <edge to="n119" from="n119">
            <attr name="label">
                <string>1</string>
            </attr>
        </edge>
        <edge to="n124" from="n121">
            <attr name="label">
                <string>this</string>
            </attr>
        </edge>
        <edge to="n115" from="n115">
            <attr name="label">
                <string>5</string>
            </attr>
        </edge>
        <edge to="n117" from="n111">
            <attr name="label">
                <string>caller</string>
            </attr>
        </edge>
        <edge to="n121" from="n121">
            <attr name="label">
                <string>append</string>
            </attr>
        </edge>
        <edge to="n117" from="n121">
            <attr name="label">
                <string>caller</string>
            </attr>
        </edge>
        <edge to="n118" from="n124">
            <attr name="label">
                <string>next</string>
            </attr>
        </edge>
        <edge to="n110" from="n121">
            <attr name="label">
                <string>x</string>
            </attr>
        </edge>
        <edge to="n110" from="n110">
            <attr name="label">
                <string>4</string>
            </attr>
        </edge>
        <edge to="n111" from="n111">
            <attr name="label">
                <string>append</string>
            </attr>
        </edge>
        <edge to="n117" from="n117">
            <attr name="label">
                <string>root</string>
            </attr>
        </edge>
        <edge to="n124" from="n111">
            <attr name="label">
                <string>this</string>
            </attr>
        </edge>
        <edge to="n122" from="n122">
            <attr name="label">
                <string>10</string>
            </attr>
        </edge>
        <edge to="n112" from="n120">
            <attr name="label">
                <string>val</string>
            </attr>
        </edge>
        <edge to="n125" from="n113">
            <attr name="label">
                <string>val</string>
            </attr>
        </edge>
        <edge to="n113" from="n118">
            <attr name="label">
                <string>next</string>
            </attr>
        </edge>
        <edge to="n121" from="n121">
            <attr name="label">
                <string>control</string>
            </attr>
        </edge>
        <edge to="n123" from="n123">
            <attr name="label">
                <string>append</string>
            </attr>
        </edge>
        <edge to="n115" from="n123">
            <attr name="label">
                <string>x</string>
            </attr>
        </edge>
        <edge to="n123" from="n123">
            <attr name="label">
                <string>control</string>
            </attr>
        </edge>
        <edge to="n116" from="n111">
            <attr name="label">
                <string>x</string>
            </attr>
        </edge>
        <edge to="n111" from="n111">
            <attr name="label">
                <string>control</string>
            </attr>
        </edge>
        <edge to="n125" from="n125">
            <attr name="label">
                <string>3</string>
            </attr>
        </edge>
        <edge to="n124" from="n123">
            <attr name="label">
                <string>this</string>
            </attr>
        </edge>
        <edge to="n116" from="n116">
            <attr name="label">
                <string>2</string>
            </attr>
        </edge>
        <edge to="n124" from="n117">
            <attr name="label">
                <string>list</string>
            </attr>
        </edge>
        <edge to="n114" from="n113">
            <attr name="label">
                <string>next</string>
            </attr>
        </edge>
        <edge to="n120" from="n114">
            <attr name="label">
                <string>next</string>
            </attr>
        </edge>
        <edge to="n116" from="n118">
            <attr name="label">
                <string>val</string>
            </attr>
        </edge>
        <edge to="n117" from="n123">
            <attr name="label">
                <string>caller</string>
            </attr>
        </edge>
        <edge to="n122" from="n114">
            <attr name="label">
                <string>val</string>
            </attr>
        </edge>
        <edge to="n119" from="n124">
            <attr name="label">
                <string>val</string>
            </attr>
        </edge>
    </graph>
</gxl>
