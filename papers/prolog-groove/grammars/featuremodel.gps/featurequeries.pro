...
target_graph_of_rule(Rule,Graph) :-
 state(State),
 state_graph(State,Graph),
 state_transition(State,Transition),
 transition_event(Transition,Event),
 ruleevent_rule(Event,Rule).

selected_feature(Graph, Feature) :-
 node_self_edges(Graph,Node,['selected']), label('name',Label), node_with_attribute(Graph,Node,Label,Feature).
