...
public class OutEdgeTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < TIMES; i++) {
            JavaGraph javagraph = new JavaGraph("C:\\Users\\Sven\\Documents\\GitHub\\Groove\\grammars\\outEdge.gps");
            ...
            Test.nodes.clear();
            Generator generator = new Generator("-r=" + AMOUNT, "-s=linear", "-a=any", "C:\\Users\\Sven\\Documents\\GitHub\\Groove\\grammars\\outEdge.gps");
            long grooveStart = System.nanoTime();
            generator.start();
            long grooveEnd = System.nanoTime();
            grooveTime.add(grooveEnd - grooveStart);
        }
        double javaAvg = javaTime.stream().mapToLong(Long::longValue).skip(5).average().orElse(0);
        double grooveAvg = grooveTime.stream().mapToLong(Long::longValue).skip(5).average().orElse(0);
