...
public class Simple {
    public static void main(String[] args) throws FormatException {
        ...
        RuleNode source = ruleFactory.nodes(graph.getTypeGraph().getFactory().getTopNode(), false, Collections.emptyList()).createNode();
        lhs.addNode(source);
        RuleNode target = ruleFactory.nodes(graph.getTypeGraph().getFactory().getTopNode(), false, Collections.emptyList()).createNode();
        lhs.addNode(target);
        ...
        RuleToHostMap ruleToHostMap = hostFactory.createRuleToHostMap();
        ruleToHostMap.putNode(source, edgeExampleNode);
        ruleToHostMap.putNode(target, nodeExampleNode);

//        Proof proof = rule.getMatch(graph, null);
        Proof proof = new Proof(condition, ruleToHostMap);
